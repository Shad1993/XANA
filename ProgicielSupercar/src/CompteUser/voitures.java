...
public class voitures extends JFrame {
	...
	public static void main(String login) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					voitures window = new voitures(login);
	...
	private final Action action = new SwingAction();
	 ...
	 public void Connect()
		...
		public void table_loadAnnuel()
	    {
	    	try 
	    	{
		    pst = con.prepareStatement("SELECT * FROM voitures vo WHERE  DateMiseEnVente >= Date_add(Now(),interval - 12 month)");
		    rs = pst.executeQuery();
		    table.setModel(DbUtils.resultSetToTableModel(rs));
		} 
	    	catch (SQLException e) 
		...
		public void table_loadJ()
	    {
	    	try 
	    	{
		    pst = con.prepareStatement("SELECT * FROM voitures WHERE DateMiseEnVente= CURRENT_DATE");
		    rs = pst.executeQuery();
		    table.setModel(DbUtils.resultSetToTableModel(rs));
		} 
	    	catch (SQLException e) 
		...
		public void table_loadM()
	    {
	    	try 
	    	{
		    pst = con.prepareStatement("SELECT * FROM voitures vo WHERE MONTH(DateMiseEnVente) = MONTH(CURRENT_DATE()) AND YEAR(DateMiseEnVente) = YEAR(CURRENT_DATE())");
		    rs = pst.executeQuery();
		    table.setModel(DbUtils.resultSetToTableModel(rs));
		} 
	    	catch (SQLException e) 
		...
		public void table_loadH()
	    {
	    	try 
	    	{
		    pst = con.prepareStatement("SELECT  * FROM voitures WHERE DateMiseEnVente >= DATE_SUB(NOW(), INTERVAL 1 WEEK) GROUP BY WEEK(DateMiseEnVente)");
		    rs = pst.executeQuery();
		    table.setModel(DbUtils.resultSetToTableModel(rs));
		} 
	    	catch (SQLException e) 
	...
	private void initialize(String login) {
		...
		frmAdmin.setTitle("Voiures");
		frmAdmin.setBounds(100, 100, 561, 541);
		frmAdmin.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		
		JLabel lblGestionDesCommandes = new JLabel("Bilan des Voitures");
